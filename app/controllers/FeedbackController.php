<?php 
/**
 * Feedback Page Controller
 * @category  Controller
 */
class FeedbackController extends BaseController{
	function __construct(){
		parent::__construct();
		$this->tablename = "feedback";
	}
	/**
     * List page records
     * @param $fieldname (filter record by a field) 
     * @param $fieldvalue (filter field value)
     * @return BaseView
     */
	function index($fieldname = null , $fieldvalue = null){
		$request = $this->request;
		$db = $this->GetModel();
		$tablename = $this->tablename;
		$fields = array("id", 
			"customer_name", 
			"customer_phone", 
			"customer_email", 
			"visit_date", 
			"department", 
			"rating", 
			"hotel_branch");
		$pagination = $this->get_pagination(MAX_RECORD_COUNT); // get current pagination e.g array(page_number, page_limit)
		//search table record
		if(!empty($request->search)){
			$text = trim($request->search); 
			$search_condition = "(
				feedback.id LIKE ? OR 
				feedback.customer_name LIKE ? OR 
				feedback.customer_phone LIKE ? OR 
				feedback.customer_email LIKE ? OR 
				feedback.visit_date LIKE ? OR 
				feedback.department LIKE ? OR 
				feedback.rating LIKE ? OR 
				feedback.feedback LIKE ? OR 
				feedback.submitted_on LIKE ? OR 
				feedback.hotel_branch LIKE ?
			)";
			$search_params = array(
				"%$text%","%$text%","%$text%","%$text%","%$text%","%$text%","%$text%","%$text%","%$text%","%$text%"
			);
			//setting search conditions
			$db->where($search_condition, $search_params);
			 //template to use when ajax search
			$this->view->search_template = "feedback/search.php";
		}
		if(!empty($request->orderby)){
			$orderby = $request->orderby;
			$ordertype = (!empty($request->ordertype) ? $request->ordertype : ORDER_TYPE);
			$db->orderBy($orderby, $ordertype);
		}
		else{
			$db->orderBy("feedback.id", ORDER_TYPE);
		}
		if($fieldname){
			$db->where($fieldname , $fieldvalue); //filter by a single field name
		}
		if(!empty($request->feedback_visit_date)){
			$vals = explode("-to-", str_replace(" ", "", $request->feedback_visit_date));
			$startdate = $vals[0];
			$enddate = $vals[1];
			$db->where("feedback.visit_date BETWEEN '$startdate' AND '$enddate'");
		}
		if(!empty($request->feedback_department)){
			$val = $request->feedback_department;
			$db->where("feedback.department", $val , "=");
		}
		if(!empty($request->feedback_rating)){
			$val = $request->feedback_rating;
			$db->where("feedback.rating", $val , "=");
		}
		$tc = $db->withTotalCount();
		$records = $db->get($tablename, $pagination, $fields);
		$records_count = count($records);
		$total_records = intval($tc->totalCount);
		$page_limit = $pagination[1];
		$total_pages = ceil($total_records / $page_limit);
		if(	!empty($records)){
			foreach($records as &$record){
				$record['visit_date'] = human_date($record['visit_date']);
			}
		}
		$data = new stdClass;
		$data->records = $records;
		$data->record_count = $records_count;
		$data->total_records = $total_records;
		$data->total_page = $total_pages;
		if($db->getLastError()){
			$this->set_page_error();
		}
		$page_title = $this->view->page_title = "Customer Feedback";
		$this->view->report_filename = date('Y-m-d') . '-' . $page_title;
		$this->view->report_title = $page_title;
		$this->view->report_layout = "report_layout.php";
		$this->view->report_paper_size = "A4";
		$this->view->report_orientation = "portrait";
		$this->render_view("feedback/list.php", $data); //render the full page
	}
	/**
     * View record detail 
	 * @param $rec_id (select record by table primary key) 
     * @param $value value (select record by value of field name(rec_id))
     * @return BaseView
     */
	function view($rec_id = null, $value = null){
		$request = $this->request;
		$db = $this->GetModel();
		$rec_id = $this->rec_id = urldecode($rec_id);
		$tablename = $this->tablename;
		$fields = array("id", 
			"customer_name", 
			"customer_phone", 
			"customer_email", 
			"hotel_branch", 
			"visit_date", 
			"department", 
			"rating", 
			"feedback", 
			"submitted_on");
		if($value){
			$db->where($rec_id, urldecode($value)); //select record based on field name
		}
		else{
			$db->where("feedback.id", $rec_id);; //select record based on primary key
		}
		$record = $db->getOne($tablename, $fields );
		if($record){
			$record['visit_date'] = human_date($record['visit_date']);
$record['submitted_on'] = human_datetime($record['submitted_on']);
			$page_title = $this->view->page_title = "Customer Feedback";
		$this->view->report_filename = date('Y-m-d') . '-' . $page_title;
		$this->view->report_title = $page_title;
		$this->view->report_layout = "report_layout.php";
		$this->view->report_paper_size = "A4";
		$this->view->report_orientation = "portrait";
		}
		else{
			if($db->getLastError()){
				$this->set_page_error();
			}
			else{
				$this->set_page_error("No record found");
			}
		}
		return $this->render_view("feedback/view.php", $record);
	}
	/**
     * Insert new record to the database table
	 * @param $formdata array() from $_POST
     * @return BaseView
     */
	function add($formdata = null){
		if($formdata){
			$db = $this->GetModel();
			$tablename = $this->tablename;
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("customer_name","customer_phone","customer_email","hotel_branch","visit_date","department","rating","feedback");
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'customer_name' => 'required',
				'customer_phone' => 'required',
				'customer_email' => 'required|valid_email',
				'hotel_branch' => 'required',
				'visit_date' => 'required',
				'department' => 'required',
				'rating' => 'required',
				'feedback' => 'required',
			);
			$this->sanitize_array = array(
				'customer_name' => 'sanitize_string',
				'customer_phone' => 'sanitize_string',
				'customer_email' => 'sanitize_string',
				'hotel_branch' => 'sanitize_string',
				'visit_date' => 'sanitize_string',
				'department' => 'sanitize_string',
				'rating' => 'sanitize_string',
				'feedback' => 'sanitize_string',
			);
			$this->filter_vals = true; //set whether to remove empty fields
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$rec_id = $this->rec_id = $db->insert($tablename, $modeldata);
				if($rec_id){
					$this->set_flash_msg("Feedback submitted successfully", "success");
					return	$this->redirect("feedback");
				}
				else{
					$this->set_page_error();
				}
			}
		}
		$page_title = $this->view->page_title = "Submit New Feedback";
		$this->render_view("feedback/add.php");
	}
	/**
     * Update table record with formdata
	 * @param $rec_id (select record by table primary key)
	 * @param $formdata array() from $_POST
     * @return array
     */
	function edit($rec_id = null, $formdata = null){
		$request = $this->request;
		$db = $this->GetModel();
		$this->rec_id = $rec_id;
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","customer_name","customer_phone","customer_email","hotel_branch","visit_date","department","rating","feedback");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'customer_name' => 'required',
				'customer_phone' => 'required',
				'customer_email' => 'required|valid_email',
				'hotel_branch' => 'required',
				'visit_date' => 'required',
				'department' => 'required',
				'rating' => 'required',
				'feedback' => 'required',
			);
			$this->sanitize_array = array(
				'customer_name' => 'sanitize_string',
				'customer_phone' => 'sanitize_string',
				'customer_email' => 'sanitize_string',
				'hotel_branch' => 'sanitize_string',
				'visit_date' => 'sanitize_string',
				'department' => 'sanitize_string',
				'rating' => 'sanitize_string',
				'feedback' => 'sanitize_string',
			);
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("feedback.id", $rec_id);;
				$bool = $db->update($tablename, $modeldata);
				$numRows = $db->getRowCount(); //number of affected rows. 0 = no record field updated
				if($bool && $numRows){
					$this->set_flash_msg("Feedback updated successfully", "success");
					return $this->redirect("feedback");
				}
				else{
					if($db->getLastError()){
						$this->set_page_error();
					}
					elseif(!$numRows){
						//not an error, but no record was updated
						$page_error = "No record updated";
						$this->set_page_error($page_error);
						$this->set_flash_msg($page_error, "warning");
						return	$this->redirect("feedback");
					}
				}
			}
		}
		$db->where("feedback.id", $rec_id);;
		$data = $db->getOne($tablename, $fields);
		$page_title = $this->view->page_title = "Edit  Feedback";
		if(!$data){
			$this->set_page_error();
		}
		return $this->render_view("feedback/edit.php", $data);
	}
	/**
     * Delete record from the database
	 * Support multi delete by separating record id by comma.
     * @return BaseView
     */
	function delete($rec_id = null){
		Csrf::cross_check();
		$request = $this->request;
		$db = $this->GetModel();
		$tablename = $this->tablename;
		$this->rec_id = $rec_id;
		//form multiple delete, split record id separated by comma into array
		$arr_rec_id = array_map('trim', explode(",", $rec_id));
		$db->where("feedback.id", $arr_rec_id, "in");
		$bool = $db->delete($tablename);
		if($bool){
			$this->set_flash_msg("Record deleted successfully", "success");
		}
		elseif($db->getLastError()){
			$page_error = $db->getLastError();
			$this->set_flash_msg($page_error, "danger");
		}
		return	$this->redirect("feedback");
	}
}
